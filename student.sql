...
#where
select student_name from students where city='pune';
#order by
select*from students order by age desc;
#group by
select course_id,AVG(marks) as avg_marks from marks group by course_id;
#having
select course_id,AVG(MARKS) AS avg_marks from marks group by course_id having AVG(marks)< 70;
#inner join
...
#view creation
create view student_city as select student_id from students where city='pune';
alter view student_city as select student_id,student_name from students where city='pune';
select*from student_city;
drop view student_city;
#delimiter data delete procedure
delimiter //
...
#insert data
DELIMITER //
create procedure insert_students(in p_id INT,p_name varchar(50),a INT,Co varchar(50),c varchar(20))
BEGIN
insert into students(student_id,student_name,age,course,city) values(p_id,p_name,a,Co,c);
END //
DELIMITER ;
#top 5 students
SELECT*from students s join marks m on s.student_id=m.student_id order by m.marks DESC LIMIT 5;
